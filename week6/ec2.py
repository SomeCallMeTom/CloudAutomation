...
if __name__ == '__main__':
    try:
        ...
        if error.response['Error']['Code'] == 'InvalidParameterValue':
            if "groupId" in error.response["Error"]["Message"]:
                print("Please validate the WebSG security group is available for use")
            else:
                print(error.response['Error']['Message'])
        elif error.response['Error']['Code'] == 'UnauthorizedOperation':
            print("Please validate your region configuration in your aws config")
        else:
            print(f"{error}")
